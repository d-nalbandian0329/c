...
static void depression(Unit all_unit[NUM], Manage *manager, int idx){
  ...
  while(i < CN){
    ...
    while(j < getSize(list)){
      setData(list, j, (Data)(getData(list, j)*0.1));
      ++j;
    }
...
static void manageStep(Unit all_unit[NUM], Manage *manager){
  ...
  sprintf(fname, "/home/takeshi/NA/CSV/MCNN/NN_DATA/HIST/MCNN_HIST_CN%dSIGMA%.3f.csv", CN, SIGMA);
  ...
  */
  ...
  max_fire = 0;
  for(i = 0; i < NUM; i++)
    if(all_unit[i].d_flag)
      ++max_fire;

  printf("Depression Unit : %d\n", max_fire);
