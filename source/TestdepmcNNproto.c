...
static void fire(Unit all_unit[NUM], Manage *manager, int idx, FILE *fp);
...
int main(void){
  ...
  for(SIGMA = 1.1; SIGMA < 2.01; SIGMA += 10000){
    init(all_unit, &manager);

    makeNet(all_unit);

    manageStep(all_unit, &manager);

    terminate(all_unit, &manager);
  }
...
static void manageStep(Unit all_unit[NUM], Manage *manager){
  ...
  sprintf(fname, "/Users/iwaitoshiya/Desktop/MCNN_DATA/U%d/CN%d/Depression/BUFFER/MCNN_DEP%.3f_U%d_T%d_RASTER_CN%dSIGMA%.3f.csv"
               , NUM, CN, DEP, NUM, C_LIMIT, CN, SIGMA);
